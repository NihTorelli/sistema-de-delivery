...
if($valor == ''){
	echo "Preencha o Valor do Produto";
	exit();
}

if($descricao_longa == ''){
	echo "Preencha a Descrição Longa do Produto!";
	exit();
}
...
if($reg_antigo != $nome){
 ...
 if ($_FILES['foto']['name'] == ""){
 	...
 	$res = $pdo->prepare("UPDATE produtos SET nome = :nome, descricao = :descricao, descricao_longa = :descricao_longa, valor = :valor, categoria = :categoria, imagem = :imagem, nome_url = :nome_url, combo = :combo where id = :id");
